...
def test_cps_notable_self_heal():
    """A CPS entry whose Notable was lost (e.g. `claude -p` timeout — 2026-07-29..31) gets its
    prose regenerated by the self-heal pass on a later run, idempotently, and a self-heal
    failure never breaks the run. Regression for the orphaned-Notable gap (the Notable is
    best-effort/off the done-gate, so the main loop never revisits it)."""
    with tempfile.TemporaryDirectory() as td:
        tdp = Path(td)
        m, c, b = (tdp / n for n in ("metrics-logs.md", "credit-put-spreads.md", "daily-briefings.md"))
        for src, dst in ((HIST / "metrics-logs.md", m), (HIST / "daily-briefings.md", b)):
            dst.write_text(src.read_text())
        # Guarantee >=1 orphan regardless of live-file state: strip the newest entry's Notable.
        ct = (HIST / "credit-put-spreads.md").read_text()
        newest = re.search(r"^##\s+(\d{4}-\d{2}-\d{2})\b", ct, re.M).group(1)
        et = parser.entry_text(HIST / "credit-put-spreads.md", newest) or ""
        stripped = re.sub(r"\n+\*\*Notable:\*\*.*?(?=\n---|\Z)", "", et, flags=re.S)
        c.write_text(ct.replace(et, stripped))
        api = parser.last_logged_date(m)  # up-to-date v1 -> only the self-heal passes do work

        kw = dict(metrics_path=m, cps_path=c, briefings_path=b, api_date=api,
                  np_latest={"tickers": []}, cps_latest=None,
                  load_backfill=lambda d: (None, None, None), no_claude=False, verbose=False,
                  briefing_fn=_pass_briefing, notable_fn=lambda *a, **k: "healed note.")
        s = orch.run(**kw)
        _ok("self-heal wrote the stripped Notable", newest in s["notables_written"])
        _ok("Notable prose present in the entry",
            "**Notable:** healed note." in (parser.entry_text(c, newest) or ""))
        s2 = orch.run(**kw)
        _ok("re-run heals nothing (idempotent)", s2["notables_written"] == [])

        # isolation: a raising notable_fn is swallowed, the run completes
        c.write_text(ct.replace(et, stripped))  # re-orphan

        def _boom(*a, **k):
            raise RuntimeError("notable disabled in this test")
        s3 = orch.run(**{**kw, "notable_fn": _boom})
        _ok("self-heal failure swallowed (run completed)", s3["notables_written"] == [])
        _ok("entry still pending, not corrupted",
            "**Notable:**" not in (parser.entry_text(c, newest) or ""))
