...
| *(WS2a, 2026-09-03)* `rv_fwd_21_cc, rv_fwd_21, capture_30d, capture_30d_log` REAL, `capture_resolved_at` TEXT | nullable | **Forward realized capture** (spec E3, Module G's `capture_30d`): for ticker-day t, `capture_30d = (IV30² − RV_cc(t→t+21)²)·10⁴` (`theta_core.realized_capture`), close-to-close like `trades.capture`; `rv_fwd_21` is the GK+overnight forward vol (the forecaster's own target). Filled with a **21-session lag** by `backend/capture.py:fill_resolved` (nightly after the shadow step, bounded; one-off `python capture.py --backfill` for history). Aggregated by `get_capture_summary()` onto `/api/shadow/summary` (additive). Advisory — never read by any decision path. |
| *(WS4, 2026-09-03)* `fvrp_ratio_trail` REAL, `veto_disagree` INTEGER | nullable | `theta_core.fvrp_veto_ratio` = `IV30 / max(sigma_fwd, rv30/100)` — the conservative veto ratio, and a 1/0 marker for rows where it and `fvrp_ratio` disagree on eligibility (Test T1's population). Gates only under `CONFIG["veto_denominator"] == "max"` (default `"sigma_fwd"`, ADR-015). `get_veto_summary()` → `veto_disagree_rate` on `/api/shadow/summary`. |
