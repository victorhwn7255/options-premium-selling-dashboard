...
**Forward-capture fill** (Signal-Quality WS2a, 2026-09-03 — a sibling step right after the shadow block, same `try/except` isolation): `capture.fill_resolved()` fills `daily_iv.capture_30d` (+ `rv_fwd_21_cc`, `rv_fwd_21`, `capture_30d_log`, `capture_resolved_at`) for every ticker-day whose 21 forward sessions of bars now exist — the spec-E3 per-ticker-day realized capture, bounded per run and idempotent. `GET /api/shadow/summary` merges `get_capture_summary(capture_window)` additively (fields `capture_*`, `sigma_fwd_log_mae`, `rv30_log_mae`); the automation renders a guarded tail segment and the v2 briefing prompt defines it. Advisory metric — no decision path reads it.

**Trade-journal mark step** (J1, 2026-07-16 — after the CPS build, same isolation contract): tickers with open positions get their in-memory chains harvested during the loop (`journal_chain_inputs` — no second data pass); post-loop, `positions_api.mark_open_positions_from_scan` writes one `position_marks` row per open position (chain lookup → targeted `get_option_quote` fallback → carried mark, flagged) plus the partial `portfolio_daily` row. `try/except`-isolated — a mark failure can never touch the v1 scan; empty book = no-op. The journal API itself lives in `backend/positions_api.py` (router `/api/positions*`, `/api/journal*`, `/api/settings*`), every route behind `auth.require_owner` (fail-closed 403; see `deployment.md § Trade-journal privacy`). The frontend Journal tab probes access on mount and renders a locked card on 403 — the public demo never sees the book.
